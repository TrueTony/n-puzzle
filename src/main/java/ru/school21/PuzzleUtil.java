...
public class PuzzleUtil {
	...
	public static Puzzle validate(String[] puz) {
		...
		for (String s : correctPuzzle) {
			for (int i = 0; i < size; i++) {
				...
				if (x >= size * size || x < 0) {
					throw new RuntimeException("The value of the puzzle is greater than the maximum possible or negative");
				}
				initialState[z][i] = x;
			}
			z++;
	...
	private static String[] check(String[] puzzle) {
		for (String s : puzzle) {
			if (s.matches("\\d")) {
				...
				throw new NotFoundException("The file did not pass verification");
			}
		}
	...
	private static void addString(String[] puzzle, String s) {
		...
		if (split.length == size) {
			...
			throw new RuntimeException("Wrong size of puzzle");
		}
	}
}
